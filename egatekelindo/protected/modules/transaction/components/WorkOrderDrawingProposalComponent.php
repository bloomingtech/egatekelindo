...
class WorkOrderDrawingProposalComponent extends CComponent {
    ...
    public function validate() {
        ...
        if (count($this->details) > 0) {
            foreach ($this->details as $detail) {
                $fields = array('date_delivery','date_return');
    ...
    public function flush() {
        ...
        foreach ($this->details as $detail) {
            ...
            if ($detail->isNewRecord) {
                if ($detail->date_delivery == NULL &&  $detail->date_return == NULL) {
                    continue;
                }
                ...
                if ((int) $detail->is_inactive === 1) {
                    $detail->delete();
                    continue;
                } else if ($detail->date_delivery == NULL) {
                    continue;
                }
                else
                    $valid = $valid && $detail->save(false);
