...
<table style="border: 1px solid">
    ...
    <?php if ($budgetingDetail->detailAccesories): ?>
        <?php foreach ($budgetingDetail->detailAccesories as $i => $detail): ?>
            <?php if ($detail->is_inactive == 0): ?>
                <tr>
                    ...
                    <td style="text-align: center;">
                        ...
                        echo CHtml::activeTextField($detail, "[$i]quantity", array('size' => 5, 'maxlength' => 10,
                            'onchange' => CHtml::ajax(array(
                                'type' => 'POST',
                                'dataType' => 'JSON',
                                'url' => CController::createUrl('ajaxJsonTotalAccesories', array('id' => $budgetingDetail->header->id, 'index' => $i, 'saleOrderDetailId' => $saleOrderDetail->id)),
                                'success' => 'function(data) {
							$("#total_accesories_' . $i . '").html(data.total);
                    ...
                    <td style="text-align: right;">
                        <span id="total_accesories_<?php echo $i; ?>">
                        ...
                        <?php if ($detail->isNewRecord): ?>
                            ...
                            echo CHtml::button('Delete', array(
                                'onclick' => CHtml::ajax(array(
                                    'type' => 'POST',
                                    'url' => CController::createUrl('ajaxHtmlRemoveDetailAccesories', array('id' => $budgetingDetail->header->id, 'index' => $i)),
        ...
        <tr style="background-color: aquamarine">
            ...
            <td style="font-weight: bold; text-align: right;" colspan="5">Sub Total Accesories</td>
            <td style="font-weight: bold; text-align: right;">
                <span id="sub_total_accesories">
                    <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $budgetingDetail->header->getTotalAccesories($saleOrderDetail->id))); ?>
                </span>
            </td>
            <td></td>
        </tr>
